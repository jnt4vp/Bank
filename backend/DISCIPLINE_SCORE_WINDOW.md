...
- The discipline window opens on the **first successful Plaid sync** where the user previously had **zero** rows in `transactions` (first linked bank only). The cutoff is the **wall-clock completion** of that sync (`datetime.now(UTC)`), not a timestamp taken from imported rows. If the DB batch’s `max(created_at)` is still ≥ that instant (same microsecond), the cutoff bumps to `max(created_at) + 1µs` so the whole import stays **out** of the window.
- **Second (or later) bank:** if `prior_txn_count > 0`, the cutoff is **not** changed.
